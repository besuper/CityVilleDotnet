...
internal sealed class Plow(CityVilleDbContext context) : AmfService<PlowRequest>
{
    public override async Task<ASObject> HandlePacket(PlowRequest request, Guid playerId, CancellationToken cancellationToken)
    {
        ...
        if ((obj.State == WorldObjectState.Planted || obj.State == WorldObjectState.Grown) && !obj.IsWithered())
            throw new DomainException(GameErrorType.InvalidState);

        var settings = GameSettingsManager.Instance.GetSettings();

        if (settings.PlowCost > 0)
            player.RemoveCoins(settings.PlowCost);

        if (settings.PlowXp > 0)
            player.AddXp(settings.PlowXp);
...
public class PlowRequest
{
    [AmfParam(1)] public BuildingPlowRequest Building { get; set; } = new();
}

public class BuildingPlowRequest
{
