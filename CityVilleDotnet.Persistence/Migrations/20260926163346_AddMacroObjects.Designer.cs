// <auto-generated />
using System;
using CityVilleDotnet.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace CityVilleDotnet.Persistence.Migrations
{
    [DbContext(typeof(CityVilleDbContext))]
    [Migration("20260926163346_AddMacroObjects")]
    partial class AddMacroObjects
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.12")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreationDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAdmin")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Collection", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Completed")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("Collection");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.CollectionItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("int");

                    b.Property<int?>("CollectionId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.HasKey("Id");

                    b.HasIndex("CollectionId");

                    b.ToTable("CollectionItem");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Coupon", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int?>("WorldFlatId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("Coupon");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.CrewMember", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("WorldObjectId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.HasIndex("WorldObjectId");

                    b.ToTable("CrewMember");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Franchise", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("FranchiseName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("FranchiseType")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<long>("TimeLastCollected")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("Franchise");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.FranchiseLocation", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CommodityLeft")
                        .HasColumnType("int");

                    b.Property<int>("CommodityMax")
                        .HasColumnType("int");

                    b.Property<int>("CustomersServed")
                        .HasColumnType("int");

                    b.Property<int>("FranchiseId")
                        .HasColumnType("int");

                    b.Property<string>("FranchiseName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("MoneyCollected")
                        .HasColumnType("int");

                    b.Property<string>("ObjectId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("StarRating")
                        .HasColumnType("int");

                    b.Property<long>("TimeLastCollected")
                        .HasColumnType("bigint");

                    b.Property<long>("TimeLastOperated")
                        .HasColumnType("bigint");

                    b.Property<long>("TimeLastSupplied")
                        .HasColumnType("bigint");

                    b.Property<string>("Uid")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.HasKey("Id");

                    b.HasIndex("FranchiseId");

                    b.ToTable("FranchiseLocation");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Friend", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("EnergyLeft")
                        .HasColumnType("int");

                    b.Property<Guid>("FriendPlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<long>("LastEnergyLeftReset")
                        .HasColumnType("bigint");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Requested")
                        .HasColumnType("bit");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("FriendPlayerId");

                    b.HasIndex("PlayerId");

                    b.HasIndex("Status");

                    b.ToTable("Friend");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.IncentivizedExpansion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ExpansionId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("FailureCount")
                        .HasColumnType("int");

                    b.Property<bool>("IsCompleted")
                        .HasColumnType("bit");

                    b.Property<long?>("StartTimestamp")
                        .HasColumnType("bigint");

                    b.Property<int>("WorldId")
                        .HasColumnType("int");

                    b.Property<int?>("X")
                        .HasColumnType("int");

                    b.Property<int?>("Y")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldId");

                    b.ToTable("IncentivizedExpansion");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.InventoryItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("StorageType")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int?>("StoredObjectId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.HasIndex("StoredObjectId");

                    b.ToTable("InventoryItem");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.LicenseItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("LicenseItem");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.LotOrder", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("ConstructionCount")
                        .HasColumnType("int");

                    b.Property<long>("LastTimeReminded")
                        .HasColumnType("bigint");

                    b.Property<int>("LotId")
                        .HasColumnType("int");

                    b.Property<int?>("OffsetX")
                        .HasColumnType("int");

                    b.Property<int?>("OffsetY")
                        .HasColumnType("int");

                    b.Property<string>("OrderResourceName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("OrderState")
                        .HasColumnType("int");

                    b.Property<int>("OrderType")
                        .HasColumnType("int");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("RecipientId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ResourceType")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("SenderId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<long>("TimeSent")
                        .HasColumnType("bigint");

                    b.Property<int>("TransmissionStatus")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.HasIndex("RecipientId");

                    b.HasIndex("SenderId");

                    b.HasIndex("OrderState", "TransmissionStatus");

                    b.ToTable("LotOrder");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.MacroObject", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("ParentItemName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("WorldId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldId");

                    b.ToTable("MacroObject");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.MapRect", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Height")
                        .HasColumnType("int");

                    b.Property<int>("Width")
                        .HasColumnType("int");

                    b.Property<int>("WorldId")
                        .HasColumnType("int");

                    b.Property<int>("X")
                        .HasColumnType("int");

                    b.Property<int>("Y")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldId");

                    b.ToTable("MapRect");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Mastery", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("Level")
                        .HasColumnType("int");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("Mastery");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Player", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AppUserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("Cash")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("CreationTimestamp")
                        .HasColumnType("datetimeoffset");

                    b.Property<int>("Energy")
                        .HasColumnType("int");

                    b.Property<int>("EnergyMax")
                        .HasColumnType("int");

                    b.Property<int>("ExpansionsPurchased")
                        .HasColumnType("int");

                    b.Property<bool>("FirstDay")
                        .HasColumnType("bit");

                    b.Property<int>("Gold")
                        .HasColumnType("int");

                    b.Property<int>("Goods")
                        .HasColumnType("int");

                    b.Property<bool>("IsNew")
                        .HasColumnType("bit");

                    b.Property<int>("LastPlayedWorldType")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("LastTrackingTimestamp")
                        .HasColumnType("datetimeoffset");

                    b.Property<int>("Level")
                        .HasColumnType("int");

                    b.Property<bool>("MusicDisabled")
                        .HasColumnType("bit");

                    b.Property<int>("PremiumGoods")
                        .HasColumnType("int");

                    b.Property<string>("ProfilePictureUrl")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<int>("RollCounter")
                        .HasColumnType("int");

                    b.Property<bool>("SfxDisabled")
                        .HasColumnType("bit");

                    b.Property<int>("Snuid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Snuid"));

                    b.Property<int>("SocialLevel")
                        .HasColumnType("int");

                    b.Property<int>("SocialXp")
                        .HasColumnType("int");

                    b.Property<long>("TimeBeforeNextEnergy")
                        .HasColumnType("bigint");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("nvarchar(32)");

                    b.Property<int>("Xp")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AppUserId");

                    b.HasIndex("Snuid");

                    b.ToTable("Player");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Quest", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("Location")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.PrimitiveCollection<string>("Progress")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.PrimitiveCollection<string>("Purchased")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("QuestType")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("Name");

                    b.HasIndex("PlayerId");

                    b.HasIndex("QuestType");

                    b.ToTable("Quest");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.SamanthaWorldVersion", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("SamanthaWorldVersion");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.SeenFlag", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("SeenFlag");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.TrainOrder", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CommodityName")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("nvarchar(32)");

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("Operation")
                        .HasColumnType("int");

                    b.Property<long>("TimeSent")
                        .HasColumnType("bigint");

                    b.Property<int>("WorldId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldId")
                        .IsUnique();

                    b.ToTable("TrainOrder");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.TrainOrderWorker", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("TrainOrderId")
                        .HasColumnType("int");

                    b.Property<int>("Zid")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("TrainOrderId");

                    b.ToTable("TrainOrderWorker");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.VisitorHelpOrder", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.PrimitiveCollection<string>("HelpTargets")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("LastTimeReminded")
                        .HasColumnType("bigint");

                    b.Property<int>("OrderState")
                        .HasColumnType("int");

                    b.Property<int>("OrderType")
                        .HasColumnType("int");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("RecipientId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("SenderId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<long>("TimeSent")
                        .HasColumnType("bigint");

                    b.Property<int>("TransmissionStatus")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.HasIndex("RecipientId");

                    b.HasIndex("SenderId");

                    b.HasIndex("OrderState", "TransmissionStatus");

                    b.ToTable("VisitorHelpOrder");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.World", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("MacroObjectIdCounter")
                        .HasColumnType("int");

                    b.Property<int>("NextBuildingId")
                        .HasColumnType("int");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Population")
                        .HasColumnType("int");

                    b.Property<int>("PopulationCap")
                        .HasColumnType("int");

                    b.Property<int>("PopulationMax")
                        .HasColumnType("int");

                    b.Property<int>("PopulationMin")
                        .HasColumnType("int");

                    b.Property<int>("PotentialPopulation")
                        .HasColumnType("int");

                    b.Property<int>("SizeX")
                        .HasColumnType("int");

                    b.Property<int>("SizeY")
                        .HasColumnType("int");

                    b.PrimitiveCollection<string>("ThemeCollections")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Type")
                        .HasColumnType("int");

                    b.Property<string>("WorldCreated")
                        .HasMaxLength(32)
                        .HasColumnType("nvarchar(32)");

                    b.Property<string>("WorldName")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("nvarchar(32)");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId");

                    b.ToTable("World");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObject", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<long?>("ActivationTime")
                        .HasColumnType("bigint");

                    b.Property<double?>("BuildTime")
                        .HasColumnType("float");

                    b.Property<int?>("Builds")
                        .HasColumnType("int");

                    b.Property<int?>("BuiltFloorCount")
                        .HasColumnType("int");

                    b.Property<int>("ClassName")
                        .HasMaxLength(64)
                        .HasColumnType("int");

                    b.Property<string>("ContractName")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int?>("CurrentState")
                        .HasColumnType("int");

                    b.Property<bool>("Deleted")
                        .HasColumnType("bit");

                    b.Property<int>("Direction")
                        .HasColumnType("int");

                    b.Property<int>("EnergyModifier")
                        .HasColumnType("int");

                    b.Property<int?>("FinishedBuilds")
                        .HasColumnType("int");

                    b.Property<int?>("FranchiseLocationId")
                        .HasColumnType("int");

                    b.Property<bool>("GivenFreeItem")
                        .HasColumnType("bit");

                    b.Property<long?>("InactiveTime")
                        .HasColumnType("bigint");

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("ItemOwner")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("NeverOpened")
                        .HasColumnType("bit");

                    b.Property<string>("ParentMacroObjectId")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<double?>("PlantTime")
                        .HasColumnType("float");

                    b.Property<int?>("RemodelBuilds")
                        .HasColumnType("int");

                    b.Property<string>("RemodelItemName")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int?>("RequiredStages")
                        .HasColumnType("int");

                    b.Property<int?>("Stage")
                        .HasColumnType("int");

                    b.Property<int>("State")
                        .HasMaxLength(32)
                        .HasColumnType("int");

                    b.Property<int>("StreakLength")
                        .HasColumnType("int");

                    b.Property<int?>("TargetBuildingClass")
                        .HasMaxLength(64)
                        .HasColumnType("int");

                    b.Property<string>("TargetBuildingName")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("TempId")
                        .HasColumnType("int");

                    b.Property<int?>("UpgradeActionCount")
                        .HasColumnType("int");

                    b.Property<int?>("Visits")
                        .HasColumnType("int");

                    b.Property<int>("WorldFlatId")
                        .HasColumnType("int");

                    b.Property<int?>("WorldId")
                        .HasColumnType("int");

                    b.Property<int>("X")
                        .HasColumnType("int");

                    b.Property<int>("Y")
                        .HasColumnType("int");

                    b.Property<int?>("Z")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ClassName");

                    b.HasIndex("FranchiseLocationId");

                    b.HasIndex("WorldFlatId");

                    b.HasIndex("WorldId");

                    b.HasIndex("Id", "WorldFlatId")
                        .IsUnique();

                    b.ToTable("WorldObject");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectMechanicCounter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<string>("MechanicType")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("WorldObjectId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldObjectId");

                    b.ToTable("WorldObjectMechanicCounter");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectSlot", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("SlotIndex")
                        .HasColumnType("int");

                    b.Property<int>("WorldObjectId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldObjectId");

                    b.ToTable("WorldObjectSlot");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectStorageItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("WorldObjectId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldObjectId");

                    b.ToTable("WorldObjectStorageItem");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectWorker", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("WorldObjectId")
                        .HasColumnType("int");

                    b.Property<int>("Zid")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("WorldObjectId");

                    b.ToTable("WorldObjectWorker");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Collection", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("Collections")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.CollectionItem", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Collection", null)
                        .WithMany("Items")
                        .HasForeignKey("CollectionId");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Coupon", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("Coupons")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.CrewMember", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", "Player")
                        .WithMany()
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("CityVilleDotnet.Domain.Entities.WorldObject", null)
                        .WithMany("CrewMembers")
                        .HasForeignKey("WorldObjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Player");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Franchise", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("Franchises")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.FranchiseLocation", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Franchise", null)
                        .WithMany("Locations")
                        .HasForeignKey("FranchiseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Friend", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", "FriendPlayer")
                        .WithMany()
                        .HasForeignKey("FriendPlayerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", "Player")
                        .WithMany("Friends")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("FriendPlayer");

                    b.Navigation("Player");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.IncentivizedExpansion", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.World", null)
                        .WithMany("IncentivizedExpansions")
                        .HasForeignKey("WorldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.InventoryItem", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("InventoryItems")
                        .HasForeignKey("PlayerId");

                    b.HasOne("CityVilleDotnet.Domain.Entities.WorldObject", "StoredObject")
                        .WithMany()
                        .HasForeignKey("StoredObjectId");

                    b.Navigation("StoredObject");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.LicenseItem", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("Licenses")
                        .HasForeignKey("PlayerId");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.LotOrder", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("LotOrders")
                        .HasForeignKey("PlayerId");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.MacroObject", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.World", null)
                        .WithMany("MacroObjects")
                        .HasForeignKey("WorldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.MapRect", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.World", null)
                        .WithMany("MapRects")
                        .HasForeignKey("WorldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Mastery", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("Masteries")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Player", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.ApplicationUser", "AppUser")
                        .WithMany()
                        .HasForeignKey("AppUserId");

                    b.Navigation("AppUser");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Quest", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("Quests")
                        .HasForeignKey("PlayerId");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.SeenFlag", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("SeenFlags")
                        .HasForeignKey("PlayerId");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.TrainOrder", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.World", null)
                        .WithOne("TrainOrder")
                        .HasForeignKey("CityVilleDotnet.Domain.Entities.TrainOrder", "WorldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.TrainOrderWorker", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.TrainOrder", null)
                        .WithMany("Workers")
                        .HasForeignKey("TrainOrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.VisitorHelpOrder", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", null)
                        .WithMany("VisitorHelpOrders")
                        .HasForeignKey("PlayerId");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.World", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.Player", "Player")
                        .WithMany("Worlds")
                        .HasForeignKey("PlayerId");

                    b.Navigation("Player");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObject", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.FranchiseLocation", "FranchiseLocation")
                        .WithMany()
                        .HasForeignKey("FranchiseLocationId");

                    b.HasOne("CityVilleDotnet.Domain.Entities.World", null)
                        .WithMany("Objects")
                        .HasForeignKey("WorldId");

                    b.Navigation("FranchiseLocation");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectMechanicCounter", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.WorldObject", null)
                        .WithMany("MechanicCounters")
                        .HasForeignKey("WorldObjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectSlot", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.WorldObject", null)
                        .WithMany("Slots")
                        .HasForeignKey("WorldObjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectStorageItem", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.WorldObject", null)
                        .WithMany("StorageItems")
                        .HasForeignKey("WorldObjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObjectWorker", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.WorldObject", null)
                        .WithMany("Workers")
                        .HasForeignKey("WorldObjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("CityVilleDotnet.Domain.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("CityVilleDotnet.Domain.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Collection", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Franchise", b =>
                {
                    b.Navigation("Locations");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.Player", b =>
                {
                    b.Navigation("Collections");

                    b.Navigation("Coupons");

                    b.Navigation("Franchises");

                    b.Navigation("Friends");

                    b.Navigation("InventoryItems");

                    b.Navigation("Licenses");

                    b.Navigation("LotOrders");

                    b.Navigation("Masteries");

                    b.Navigation("Quests");

                    b.Navigation("SeenFlags");

                    b.Navigation("VisitorHelpOrders");

                    b.Navigation("Worlds");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.TrainOrder", b =>
                {
                    b.Navigation("Workers");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.World", b =>
                {
                    b.Navigation("IncentivizedExpansions");

                    b.Navigation("MacroObjects");

                    b.Navigation("MapRects");

                    b.Navigation("Objects");

                    b.Navigation("TrainOrder");
                });

            modelBuilder.Entity("CityVilleDotnet.Domain.Entities.WorldObject", b =>
                {
                    b.Navigation("CrewMembers");

                    b.Navigation("MechanicCounters");

                    b.Navigation("Slots");

                    b.Navigation("StorageItems");

                    b.Navigation("Workers");
                });
#pragma warning restore 612, 618
        }
    }
}
